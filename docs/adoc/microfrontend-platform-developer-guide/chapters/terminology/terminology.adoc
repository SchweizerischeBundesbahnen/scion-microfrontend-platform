...
+
An intention refers to one or more <<terminology:capability,capabilities>> that a <<terminology:micro-application,micro application>> wants to interact with. Manifesting intentions allows us to see dependencies between applications down to the functional level.
+

...
+
A capability represents some functionality of a <<terminology:micro-application,micro application>> that is available to qualified micro applications through the <<terminology:intention-api,Intention API>>. A micro application declares its capabilities in its <<terminology:manifest,manifest>>. Qualified micro applications can then browse the capabilities, or interact with provided capabilities via <<terminology:intent,intent>>.
+
A capability is formulated in an abstract way consisting of a _type_ and optionally a _qualifier_. The _type_ categorizes a capability in terms of its functional semantics. A capability may also define a _qualifier_ to differentiate different capabilities of the same _type_.
+
...
+
An intention refers to one or more <<terminology:capability,capabilities>> that a <<terminology:micro-application,micro application>> wants to interact with.
+

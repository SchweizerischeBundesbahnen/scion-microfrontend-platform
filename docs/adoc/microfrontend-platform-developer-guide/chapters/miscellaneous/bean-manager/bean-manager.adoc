...
A decorator must implement the `decorate` method of the `BeanDecorator` interface and return the proxied bean. To proxy a bean, you can create a JavaScript proxy, or create an anonymous class delegating to the actual bean. Bean decoration can be particularly useful for the `MessageClient` or `IntentClient`, e.g., if using Angular, to pipe its Observables to emit inside the Angular zone.

For an example, see the chapter <<chapter:angular-integration-guide:preparing-messaging-for-use-with-angular>>.

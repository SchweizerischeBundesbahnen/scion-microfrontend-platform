...
- <<chapter:angular-integration-guide:preparing-messageclient-for-use-with-angular>>
****
'''
...
<2> Registers microfronted routes as child routes.
...
----
<1> Waits until the platform started.
...
[[chapter:angular-integration-guide:preparing-messageclient-for-use-with-angular]]
[discrete]
=== Preparing the MessageClient for Use with Angular
====
TIP: Refer to chapter <<chapter:bean-manager:decorating-beans>> to learn more about bean decorators.
====

Messages are pushed to the message client via a separate browsing context, preventing Angular (or more precisely zone.js) from triggering a change detection cycle, causing the UI not to update as expected.

Therefore, we recommend to decorate the `MessageClient` with a bean decorator and pipe its Observables to emit inside the Angular zone, as following:

[source,typescript]
----
include::angular-zone-message-client-decorator.snippets.ts[tags=decorator]
----
<1> Obtains the `NgZone` from the bean manager.

...
IMPORTANT: We recommend starting the platform outside of the Angular zone in order to avoid excessive change detection cycles of platform-internal subscriptions to global DOM events.
...
<4> Constructs the config object to configure the platform and list applications that are allowed to interact with the platform. Refer to <<chapter:configuration:configuring-the-platform>> for more information about configuring the platform.
<5> Starts the platform host outside of the Angular zone.
...
IMPORTANT: We recommend connecting to the platform host outside of the Angular zone in order to avoid excessive change detection cycles of platform-internal subscriptions to global DOM events.
...
<2> Injects `NgZone` using Angular’s `inject` function.
<3> Registers a decorator to synchronize platform-specific RxJS Observables with the Angular Zone. See <<chapter:angular-integration-guide:synchronizing-rxjs-observables-with-angular-zone>> for more information.
<4> Connects to the platform host outside of the Angular zone.
...
The following listing illustrates how to configure an Angular resolver to connect to the platform from a micro application. Similarly, you could start the platform in the host application.

...
IMPORTANT: For Angular applications using Zone.js, start or connect to the platform outside the Angular zone to avoid excessive change detection cycles caused by platform-internal messaging. Refer to the Angular integration guide in version https://microfrontend-platform-developer-guide-v2-1-0.scion.vercel.app/#chapter:angular_integration_guide[2.1.0] for more information and examples.
...
<3> Constructs the config object to configure the platform and list applications that are allowed to interact with the platform. Refer to <<chapter:configuration:configuring-the-platform>> for more information about configuring the platform.
<4> Starts the platform host.
...
<3> Connects to the platform host, passing the symbolic name of the application as argument.
...
<2> Connects to the platform host, passing the symbolic name of the application as argument.
<3> Registers microfrontend routes as child routes.

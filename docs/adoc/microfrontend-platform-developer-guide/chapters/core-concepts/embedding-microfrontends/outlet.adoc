...
TIP: If using the <<chapter:intention-api, Intention API>>, you do not need to know the URL of the microfrontend you want to embed. Simply issue an intent for showing the microfrontend, pass the outlet name along with the intent, and let the microfrontend embed itself. See chapter <<chapter:intent-based-messaging:issuing-an-intent>> for an example.

...
** If used as qualifier property key, matches intents even if having additional property properties.
   Use it like this: `{'{asterisk}': '{asterisk}'}` +
** If used as qualifier value, requires intents to contain that property, but with any value allowed (except for `null` or `undefined` values).
...
For more information about handling an intent, see the chapter <<chapter:intent-based-messaging:handle-intent>> in <<chapter:intent-based-messaging>>.
...
For more information about issuing an intent, see the chapter <<chapter:intent-based-messaging:issue-intent>> in <<chapter:intent-based-messaging>>.
...
NOTE: To see how to handle intents in the capability providing micro application, see the chapter <<chapter:intent-based-messaging:handle-intent>> in <<chapter:intent-based-messaging>>.

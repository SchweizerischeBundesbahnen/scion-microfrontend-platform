...
A parameter has a name, can be required, and may have a default value if optional, as follows: `{"name":"param", "required": false, "default": "defaultValue"}`. A description and metadata can be included, e.g., to be evaluated in an interceptor. Parameters can be marked as deprecated, with a replacement specified.

...
<3> Looks up the `MessageClient` from the bean manager and issues the intent.
...
----
<1> Initiates a _request-response_ communication by invoking the `requestByIntent$` method on the `MessageClient`. In this example, we request the user’s access token.
...
The following code snippet shows how to reply to a request. For illustrative purposes, when receiving a request, a subscription to the auth service is created to get the user’s access token. The token Observable is a hot Observable, meaning it never completes and emits upon renewal of the token.

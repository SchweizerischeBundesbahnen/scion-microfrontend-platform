...
To explain the concept of intent-based messaging, let's imagine an eCommerce app where a micro application provides a checkout wizard that can be started via intent.

[source,json]
----
include::intent-based-messaging.snippets.ts[tags=intention-declaration]
----
<1> Declares the intention for starting the checkout wizard.
+
...
[source,typescript]
----
include::intent-based-messaging.snippets.ts[tags=issue-intent]
----
<1> Constructs the intent for starting the checkout wizard.
...
A micro application can provide functionality to other micro applications via capabilities declared in its manifest.

The following code snippet illustrates how to declare the checkout wizard capability.
...
<2> Categorizes the capability as a `wizard`.
+
There are a handful of platform-specific capability types, such as `activator` or `microfrontend`. For custom capabilities, you can choose any name.
<3> Qualifies the capability if having multiple `wizard` capabilities.
...
The following code snippet illustrates how to initiate a _request-response_ communication via intent-based messaging and receiving replies. To explain this kind of communication, we have a requestor requesting the user’s access token.
...
<2> Prints the received access token to the console.
...
_For illustration purposes, the following example subscribes to the authentication service for each intent to obtain the user's access token. The token observable never completes and continuously emits when the token is refreshed._
